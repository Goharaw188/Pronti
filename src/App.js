...
// Board configuration
const BOARD_ID = "5000295550"; // Duplicate of Pronti Sales Pipeline
const WORKSPACE_ID = "2299275";

// Fetch data from Monday.com
const fetchBoardData = async (setBoardData, setLoading, setError) => {
  try {
    setLoading(true);
    const query = `
      query {
    ...
    if (response.data && response.data.boards && response.data.boards[0]) {
      setBoardData(response.data.boards[0].items);
    }
  } catch (err) {
...
function App() {
  ...
  useEffect(() => {
    ...
    fetchBoardData(setBoardData, setLoading, setError);
  }, []);

  // Update items when boardData changes
  useEffect(() => {
    if (boardData.length > 0) {
      setItems(boardData);
      setSelectedLead(boardData[0]); // Select first lead by default
    }
  }, [boardData]);

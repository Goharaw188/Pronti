...
function App() {
  ...
  const [selectedLead, setSelectedLead] = useState(null);
      ...
      useEffect(() => {
        // Initialize Monday.com SDK integration
        const initializeMondayIntegration = async () => {
          try {
            setLoading(true);
            
            // Listen for context changes (board, view, etc.)
            monday.listen("context", async (res) => {
              console.log('Monday.com context:', res.data);
              const { boardId, viewId } = res.data;
            ...
            // Listen for real-time events (item changes, column updates, etc.)
            monday.listen("events", (res) => {
              console.log('Monday.com event:', res);
              // Handle real-time updates here
              // You can refresh data or update specific items
            });

            // Listen for settings changes
            monday.listen("settings", (res) => {
              console.log('Monday.com settings:', res);
            });

            // Listen for filter changes
            monday.listen("filters", (res) => {
              console.log('Monday.com filters:', res);
            });

            // Listen for visible item changes
            monday.listen("visibleItems", (res) => {
              console.log('Monday.com visible items:', res);
            });

          } catch (err) {
            console.error('Monday.com SDK Error:', err);
        ...
        // Fallback for non-Monday.com environments (like localhost)
        setTimeout(() => {
